...
public class DmRecyclerViewWrapper extends FrameLayout {
        ...
        if (mEnableLoadMore != enable || mIsFirstSetLoadMore) {
            ...
            if (mRvRecycler.getAdapter() != null && mRvRecycler.getAdapter() instanceof DmBaseAdapter) {
                ...
                if (enable && adapter.getCustomLoadingView() == null) {
                    View loadingView = mInflater.inflate(R.layout.dm_recyclerview_bottom_progressbar, this, false);
                    // For multi language dynamic load
                    ((TextView)loadingView.findViewById(R.id.tv_progress_loading)).setText(R.string.dm_progress_loading);

                    adapter.setCustomLoadingView(loadingView);
                }
    ...
    private RecyclerView.OnScrollListener newOnScrollListener() {
        return new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (!mIsLoadingMore && (mTotalItemCount - mVisibleItemCount) <= mFirstVisibleItem && dy > 0) {
                    //todo: there are some bugs needs to be adjusted for admob adapter
    ...
    private RecyclerView.AdapterDataObserver newAdapterDataObserver() {
        return new RecyclerView.AdapterDataObserver() {
            ...
            private void updateData() {
                ...
                if (getAdapter().getItemCount() - (getAdapter().hasLoadingView() ? 1 : 0) <= 0) {
    ...
    public boolean isRefreshing() {
        return mSrlRefresh != null && mSrlRefresh.isRefreshing();
    }
}
