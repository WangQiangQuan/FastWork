 ...
 * 
 * */
public class DmSpannableUtils {
	private DmSpannableUtils( ){
	 ...
	 * setTextSize("abc",24,0,2) = normal string
	 * */
	public static SpannableString setTextRelativeSize(String content, int startIndex, int endIndex, float proportion ){
		if( TextUtils.isEmpty( content ) || proportion <= 0 || startIndex >= endIndex || startIndex < 0 || endIndex > content.length( ) ){
			return null;
		}
		
		SpannableString spannableString = new SpannableString( content );
		spannableString.setSpan( new RelativeSizeSpan(proportion), startIndex, endIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE );
