...
public class SwipeBackLayout extends ViewGroup {
    ...
    public SwipeBackLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mTouchSlop = mDragHelper.getTouchSlop() / 2;
            ...
            case MotionEvent.ACTION_MOVE:
                if (innerScrollView != null && Util.contains(innerScrollView, downX, downY)) {
                    ...
                    if (mDirectionMode == FROM_LEFT || mDirectionMode == FROM_RIGHT) {
                        if (distanceY > mTouchSlop && distanceY > (distanceX / 2)) {
                            return super.onInterceptTouchEvent(ev);
                        }
                    } else if (mDirectionMode == FROM_TOP || mDirectionMode == FROM_BOTTOM) {
                        if (distanceX > mTouchSlop && distanceX > (distanceY / 2)) {
                            return super.onInterceptTouchEvent(ev);
                        }
                    }
                }
    ...
    public void scrollToFinishActivity() {
        final int childWidth = mDragContentView.getWidth();
        final int childHeight = mDragContentView.getHeight();

        int left = 0, top = 0;
        if (mDirectionMode == FROM_LEFT) {
            left = childWidth;
        } else if (mDirectionMode == FROM_RIGHT) {
            left = -childWidth;
        } else if (mDirectionMode == FROM_BOTTOM) {
            top = -childHeight;
        } else if (mDirectionMode == FROM_TOP) {
            top = childHeight;
        }
        mDragHelper.smoothSlideViewTo(mDragContentView, left, top);
        invalidate();
    }
